...
//PacketQueue 通过小链表AVPacketList 把音视频帧 AVPacket 组成一个顺序队列，
//是数据交换中转站，当然同步互斥控制逻辑是必不可少
...
typedef struct VideoState {
    ...
    uint8_t *audio_buf;//输出的音频缓存
    uint8_t *audio_buf1;
    unsigned int audio_buf_size; /* in bytes */ // 解码后音频数据大小
    unsigned int audio_buf1_size;
    int audio_buf_index; /* in bytes */ // 已输出音频数据大小
...
//刷新队列，释放掉队列中所有动态分配的内存，
//包括音视频裸数据占用的内存和 AVPacketList 结构占用的内存，参考上面示意图(PacketQeueue.jpg)。
static void packet_queue_flush(PacketQueue *q)
{
    ...
    for (pkt = q->first_pkt; pkt; pkt = pkt1) {
        pkt1 = pkt->next;
        av_packet_unref(&pkt->pkt);// 释放音视频数据内存
        av_freep(&pkt);// 释放 AVPacketList 结构
    }
    q->last_pkt = NULL;
    q->first_pkt = NULL;
    q->nb_packets = 0;
...
/*
下面重点分析video_refresh()函数。该函数用于将图像显示到显示器上
...
//在这个结构体中，可以看出选项的名称为“i”，选项包含选项值（HAS_ARG），选项的处理函数是opt_input_file()，选项的说明是“input file name”。
...
//强行设置设置屏幕的宽度选项（“-x”选项）：
//“-x”选项包含选项值（HAS_ARG），选项处理函数是opt_width()
//选项说明是"force displayed width"。

//全屏（“-fs”选项）
//从代码中可以看出，
static const OptionDef options[] = {
    CMDUTILS_COMMON_OPTIONS
	//从代码中可以看出，“-x”选项包含选项值（HAS_ARG），选项处理函数是opt_width()。选项说明是"force displayed width"
    { "x", HAS_ARG, { .func_arg = opt_width }, "force displayed width", "width" },
    { "y", HAS_ARG, { .func_arg = opt_height }, "force displayed height", "height" },
    { "s", HAS_ARG | OPT_VIDEO, { .func_arg = opt_frame_size }, "set frame size (WxH or abbreviation)", "size" },
	//“-fs”选项包含布尔型选项值（OPT_BOOL），并绑定了全局变量is_full_screen。
	//选项说明是"force full screen"。
...
int main(int argc, char **argv)
{
	...
	//stream_open()stream_open()的作用是打开输入的媒体。
