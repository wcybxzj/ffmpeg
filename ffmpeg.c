...
//不同在于视频编码函数avcodec_encode_video2()变成了音频编码函数avcodec_encode_audio2()。
...
av_dump_format()：在屏幕上打印输出格式信息。
注意是输出格式的信息，输入格式的信息的打印是在parse_options()函数执行过程中调用opt_input_file()的时候打印到屏幕上的。
...
process_input_packet()：解码压缩编码的数据并将之送至AVFilterContext。
*/
