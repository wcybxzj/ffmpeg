...
AVInputFormat是类似 COM 接口的数据结构， 表示输入文件容器格式，着重于功能函数，
一种文件容器格式对应一个AVInputFormat结构，在程序运行时有多个实例。
next 变量用于把所有支持的输入文件容器格式连接成链表， 便于遍历查找；
priv_data_size 标示具体的文件容器格式对应的 Context的大小， 在本例中是 AVIContext，
这些具体的结够定义散落于各个.c文件中，为避免太多的 if else 类语句判断类型再计算大小，
这里就直接指明大小， 因为这是一个编译时静态确定的字段，所以放在AVInputFormat而不是AVFormatContext 中
*/
...
typedef struct AVInputFormat {
     ...
     * Size of private data so that it can be allocated in the wrapper.
     */
	// 标示具体的文件容器格式对应的Context 的大小，在本例中是 AVIContext
    int priv_data_size;
 ...
 * AVRational time_base：avpriv_set_pts_info()函数初始化
 * 时基。通过该值可以把PTS，DTS转化为真正的时间。
 * FFMPEG其他结构体中也有这个字段，但是根据我的经验，只有AVStream中的time_base是可用的。PTS*time_base=真正的时间
 * int64_t duration：
 * 该视频/音频流长度
...
typedef struct AVStream
{
	...
	AVCodecContext *actx;
	//codec 解析器，每一种编码器在进行压缩时都会对实际负载数据进行封装，
	//加入头信息，如 h264，需要解析 nal单元，关联通过 avav_find_stream_info()
...
AVFormatContext结构表示程序运行的当前文件容器格式使用的上下文，
着重于所有文件容器共有的属性(并且是在程序运行时才能确定其值)和关联其他结构的字段。
