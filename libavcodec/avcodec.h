...
structAVCodec*codec: // 指向当前 AVCodec 的指针，
int bit_rate：平均比特率
unsigned char *extradata;// Codec 的私有数据，对Audio 是WAVEFORMATEX 结构扩展字节。
int extradata_size; 对Video 是 BITMAPINFOHEADER 后的扩展字节
...
AVCodecContext结构表示程序运行的当前Codec使用的上下文，
着重于所有Codec共有的属性(并且是在程序运行时才能确定其值)和关联其他结构的字段。
extradata和 extradata_size 两个字段表述了相应 Codec 使用的私有数据，
对Codec全局有效，通常是一些标志信息；
codec字段关联相应的编解码器；
priv_data字段关联各个具体编解码器独有的属性上下文，和AVCodec结构中的priv_data_size配对使用。
*/
typedef struct AVCodecContext {
    /**
...
* enum CodecID id; // 标示Codec 的ID，有 CODEC_ID_MSRLE， CODEC_ID_TRUESPEECH等
...
* int priv_data_size; // 标示具体的Codec 对应的Context 的大小，在本例中是MsrleContext  或TSContext的大小。
* int(*init)(AVCodecContext*);// 标示Codec 对外提供的操作
* int(*encode)(AVCodecContext*,uint8_t *buf,int buf_siz e, void *data);
* int(*close)(AVCodecContext*);
* int(*decode)(AVCodecContext*,void *outdata,int *outdata_size, uint8_t *buf,int buf_siz e);
* int capabilities; // 标示Codec 的能力，在瘦身后的 ffplay 中没太大作用，可忽略
* structAVCodec*next; // 用于把所有 Codec 串成一个链表，便于遍历
*/
...
//一种媒体类型对应一个AVCodec 结构，在程序运行时有多个实例。 
//next 变量用于把所有支持的编解码器连接成链表，便于遍历查找； 
//id 确定了唯一编解码器
//priv_data_size 表示具体的 Codec 对应的 Context 结构大小，
//比如 MsrleContext或TSContext，这些具体的结够定义散落于各个.c文件中，
//为避免太多的if else 类语句判断类型再计算大小，这里就直接指明大小， 
//因为这是一个编译时静态确定的字段，所以放在AVCodec 而不是 AVCodecContext中
typedef struct AVCodec {
    /**
