...
/*
X264_frame()调用x264_encoder_encode()完成了编码工作。

x264_encoder_encode()的输入是x264_picture_t，输出是x264_nal_t；

X264_frame()的输入是AVFrame，输出是AVPacket。

X264_frame()在调用编码函数前将AVFrame转换成了x264_picture_t，
而在调用编码函数之后调用encode_nals()将x264_nal_t转换成了AVPacket。
*/
...
AVCodec结构体ff_libx264_encoder中设定编码器初始化函数是X264_init()
...
X264_init()主要将各种选项值传递给libx264。这些选项有两个来源：
AVCodecContext和X264Context。
AVCodecContext中包含了编码器的一些通用选项，而X264Context包含了一些libx264特有的选项。
FFmpeg中的一些选项的单位和libx264中对应选项的单位是不一样的，因此需要做一些转换。
例如像素格式的转换函数convert_pix_fmt()就是完成了这个功能。

convert_pix_fmt()将AV_PIX_FMT_XXX转换成了X264_CSP_XXX。
在一切参数设置完毕后，X264_init()会调用x264_encoder_open()打开编码器，完成初始化工作。


*/

//X264_init()用于初始化libx264编码器
...
/*
libx264对应的AVCodec结构体ff_libx264_encoder中设定编码器
初始化函数是X264_init()，
编码一帧数据的函数是X264_frame()，
编码器关闭函数是X264_close()。
*/
